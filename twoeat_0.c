...
LONG lParam;
{
    ...
    case WM_SIZE:
    {
        ...
        /* 临时双缓冲在 WM_PAINT 内创建/销毁，自动适应窗口大小 */

        /* 清理旧持久化位图残留（如有），避免废弃全局变量泄漏 */
        if (hBackBitmap != NULL) {
     ...
     * 棋盘区域双缓冲 + 边矩挖洞技术：
     *
     * 核心思路：背景只画边矩，棋盘区域完全不动。
     * 这样 FillRect 和 BitBlt 之间即使屏幕刷新，棋盘区域显示的仍是
     * 上一帧的完整画面，不会暴露绿色中间态。
     *
     * 边矩四块：
     *   上边：0,0 到 宽,棋盘顶
     ...
     *   右边：棋盘右,棋盘顶 到 宽,棋盘底
     *
     * 内存位图里自己铺背景（因为屏幕棋盘区域没刷绿），然后画棋盘元素，
     * 最后 BitBlt 整块棋盘区域覆盖上去。
     * ---------------------------------------------------------------- */
    case WM_PAINT:
    {
        HDC hdcMem;
        ...
        if (hdcMem != NULL) {
            hbmMem = CreateCompatibleBitmap(hdc, boardW, boardH);
            if (hbmMem != NULL) {
                hbmOld = SelectObject(hdcMem, hbmMem);

                /* 位图里先铺背景（屏幕棋盘区域没刷绿，自己补） */
                rcMargin.left = 0; rcMargin.top = 0;
                rcMargin.right = boardW; rcMargin.bottom = boardH;
                FillRect(hdcMem, &rcMargin, hbrBackground);

                /* 偏移视口原点 */
                oldOrg = SetViewportOrg(hdcMem,
                                        -boardLayout.baseRect.left,
                                        -boardLayout.baseRect.top);

                /* 在内存 DC 上画棋盘元素 */
                DrawBoardBase(hdcMem);
                DrawGrid(hdcMem);
                DrawPieces(hdcMem);

                /* 恢复视口 */
                SetViewportOrg(hdcMem, LOWORD(oldOrg), HIWORD(oldOrg));
